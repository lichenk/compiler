...
use vars qw($LAB $COMPILER $COMPILER_EXEC $COMPILER_ARGS @LEXTS $LLVM $GCC $RUNTIME
            $REF_COMPILER $REF_COMPILER_ARGS $MAKE_TIMEOUT $COMPILER_TIMEOUT $LLVM_TIMEOUT
...
if (-e "l4lib.c") {
  $RUNTIME = "15411.c l4lib.c $rt_stem.c";
}
...
our $MAKE_TIMEOUT       = 240;  # timeout for making compiler
our $COMPILER_TIMEOUT   = 10;   # timeout for running compiler
our $LLVM_TIMEOUT       = 10;   # timeout for running llc 
...
my $MAX_SCORE0 = 0;     # maximal score for compiler, test suite 0
my $MAX_SCORE1 = 70;    # maximal score for compiler, test suite 1
my $MAX_SCORE2 = 30;    # maximal score for compiler, test suite 2
my $TESTS1_N = 10;      # first n failing suite 1 tests...
my $TESTS1_PTS = 1;     # ...are worth this many points each
my $TESTS2_N = 10;      # first n failing suite 1 tests...
my $TESTS2_PTS = 1;     # ...are worth this many points each
my $TESTS0_MIN = 0;     # number of error cases in tests0
my $TESTS1_MIN = 0;     # number of error cases in tests1
my $TESTS2_MIN = 2832;     # number of error cases in tests2
...
our $CMPL_GRADE = {
    ...
    "tests2" => sub {
        ...
        if ($succeeded >= $tried - $TESTS2_N) {
            $grade = $MAX_SCORE2 - ($tried - $succeeded) * $TESTS2_PTS;
        } elsif ($succeeded >= $TESTS2_MIN) {
            $grade = POSIX::floor(($MAX_SCORE2 - $TESTS2_N * $TESTS2_PTS)
                                  * ($succeeded - $TESTS2_MIN)
                                  / ($tried - $TESTS2_N - $TESTS2_MIN));
        } else {
            $grade = 0;
        }
