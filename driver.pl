...
sub test {
    ...
    if ($ref_compiler) {
        ...
        $command = "$COMPILER_EXEC --llvm $COMPILER_ARGS $file @Opt_Pass";
	# printf("%s\n", $command);
	    ...
	    if (-e $asm_file) {
		move($asm_file, $asm_file.".old")
		    or die "could not move $asm_file from previous compilation\n";
	    }

	    my $llvm_source = suffix($file, "ll");
	    if (-e suffix($file, "bc")) {
		$llvm_source = suffix($file, "bc");
	    }

            # probably, this doesn't belong here...
            $command = "$LLVM $llvm_source -o $asm_file";
	    # printf("%s\n", $command);
            my $timeout = 0;
            ($result, $timeout) = system_with_timeout($LLVM_TIMEOUT, $command, $write, $write);
            if ($result != 0) {
              printf("%s\n", "invocation of $LLVM failed");
            }

            move($asm_file, $asm_dest) or printf("%s\n", "couldn't move $asm_file");
