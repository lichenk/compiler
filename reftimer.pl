	...
	foreach $flag (@BENCH_FLAGS) {
		foreach $safe (@BENCH_SAFES) {
			...
			my $key = "$safe $flag";
                        ...
                        $command = "/afs/cs.cmu.edu/academic/class/15411-f13/bin/cc0bench -c '-std=gnu89 $flag' -r $safe $file -o bench";
                        ($result, $timeout) = system_with_timeout($GCC_TIMEOUT, $command, $write, $write);

                        if ($result != 0) {
                          printf("cc0 compilation failure\n");
                        } else {
