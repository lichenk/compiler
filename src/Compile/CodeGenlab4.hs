...
-}
module SafeCompile.CodeGen where

import SafeCompile.Constants
import Compile.Types
import qualified SafeCompile.MapWrap as MapWrap
import qualified Data.Map as Map
import SafeCompile.RegisterList
import Data.Tuple
import Debug.Trace
...
moveArgToTemp (allocmap, _, _, _) ((t, arg), index) =
  case Map.member arg allocmap of
    True -> AAsm [ATemp ((Map.!) allocmap arg) t] ANop [ALoc (colorInArg index t)]
...
codeGen :: (CType -> Int, CType -> String -> Int) -> AST -> [AAsm]
...
evalLvalAsExp (allocmap, n, f, s) (LvalueArrayAccess lval expr _) = 
  ...
  in
    case t of
      CStruct _ -> (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ offset ++ memloc ++ retmem, ATemp n t)
      _ -> (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ offset ++ memloc ++ deref, ATemp n t)
...
genExp (allocmap, n, f, s) (Ternary exp1 exp2 exp3 _) dest =
  let
    cogen1 = genExp (allocmap, n + 1, f, s) exp1 (ATemp n CBool)
    cogen2 = genExp (allocmap, n + 2, f, s) exp2 dest
    cogen3 = genExp (allocmap, n + 3, f, s) exp3 dest
  in
    cogen1 ++ [AIf (ACond $ ALoc $ ATemp n CBool) cogen2 cogen3]
...
genExp (allocmap, n, f, sizeof) (AllocArray t expr _) dest =
  let
    ...
    totalSizePlusLen = [AAsm [ATemp (n+3) (CPtr t)] AAdd [ALoc (ATemp (n+2) (CPtr t)), AImm 8]]
    ...
    temp = genExp (tempmap, n+4, f, sizeof) tempexp (ATemp n (CPtr t))
    writeSize = [AAsm [derefLoc (ATemp n (CPtr CInt))] ANop [ALoc (ATemp (n+1) CInt)]]
    finalPtr = [AAsm [dest] AAdd [ALoc (ATemp n (CPtr t)), AImm 8]]
  in
...
genExp (allocmap, n, f, sizeof) (ArrayAccess expr1 expr2 _) dest =
  ...
  in
    case t of
      CStruct _ -> arr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ offset ++ memloc ++ retmem
      _ -> arr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ offset ++ memloc ++ deref
...
genCtrl (allocmap, n, f, s) (If expr stmt1 stmt2) =
  let
    ...
    (newstmt1, newstmt2) = case expr of
      ...
      _ -> (stmt1, stmt2)
    cogen = genExp (allocmap, n + 1, f, s) expr (ATemp n (getExpType expr))
    rest = [AIf (ACond $ ALoc $ ATemp n CBool) (genStmt (allocmap, n + 1, f, s) newstmt1) 
...
genCtrl (allocmap, n, f, s) (While exp stmt) =
  let
    cogen = genExp (allocmap, n + 1, f, s) exp (ATemp n (getExpType exp))
    rest = [AWhile (ACond $ ALoc $ ATemp n CBool) ((genStmt (allocmap, n + 1, f, s) stmt) ++ cogen)]
  in
    cogen ++ rest
