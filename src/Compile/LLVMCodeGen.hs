module Compile.LLVMCodeGen(llvmcodeGen,fuckType,astParamsToLLVMParams,ctypeToLtype) where
...
genLval (allocmap, n, f, s, z) (LvalueArrayAccess lval expr ctype) =
  ...
  in
    (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ larray ++ larrayderef ++ memloc, derefLoc answer)
...
evalLvalAsExp (allocmap, n, f, s, z) (LvalueArrayAccess expr1 expr2 _) = 
  ...
  in
    case t of
      ...
      LStruct _ -> (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ larray ++ larrayderef ++ memloc ++ moveArrayElementPtr, typedPtrDest, LPtr t)
      -- Else, return the array element
      _ -> (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ larray ++ larrayderef ++ memloc ++ loadArrayElement, typedDest, t)
...
genExp (allocmap, n, f, s, z) dbg@(ArrayAccess expr1 expr2 _) idest =
  ...
  in
    case t of
      -- If array is an array of LStructs, return a pointer to the array element
      LStruct _ -> (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ larray ++ larrayderef ++ memloc ++ moveArrayElementPtr, LPtr t)
      -- Else, return the array element
      _ -> (buildArr ++ idx ++ arrLenAddr ++ arrLen ++ boundsBool ++ checkBool ++ larray ++ larrayderef ++ memloc ++ loadArrayElement, t)
