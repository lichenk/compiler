...
findUsedAndDefined (IAsm [dest] ACmp arg) = Set.fromList $ [Use dest] ++ (useDefSrc arg) ++ [Def (IReg (fromIntegral eflagsRegisterNumber) CNoType)]
...
useDefDest (IHeap val _) _ = 
  case val of
    ...
    ILoc (IInArg n _) -> [Use (IInArg n CNoType)]
    ILoc _ -> error ("Invalid address location3: " ++ show val)
    _ -> []
